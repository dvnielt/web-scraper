...
def fetch_url():
    url = _url.get()
    config['images'] = []
    _images.set(str(()))  # initialised as an empty tuple
    try:
...
def fetch_url_link():
    ...
    try:
        ...
        for link in soup.findAll('a'):
            src = link.get('href')
            link_url = f'{src}'
            if src[0:16] != "http://localhost":
                if src[0:4] == "http":
                    name = str(src)
                    links.append(dict(name=name, url=link_url))

        if links:
            _images.set(tuple(link['name'] for link in links))
...
if __name__ == "__main__": # execute logic if run directly
    ...
    _fetch_btn = ttk.Button(
        _url_frame, text='Fetch title', command=fetch_url_title)  # create button
    _fetch_btn.grid(row=1, column=1, sticky=W, padx=5)

    _fetch_btn = ttk.Button(
        _url_frame, text='Fetch link', command=fetch_url_link)  # create button
